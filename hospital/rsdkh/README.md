...
Resep Pergantian IGD mengambil sumber dari Planning Pengkajian Dokter IGD yang baru, bukan Assessment IGD 2. Pembaca menerima JSON lama maupun terbaru dan memakai Planning yang terlihat pada sesi tab sebagai fallback untuk nomor registrasi yang sama. Tombol `Buat Resep dari Seluruh Instruksi` di sebelah `Print` membawa semua Instruksi Dokter langsung ke Resep Elektronik V2, memilih Resep Pergantian IGD, lalu meminta depo sebelum Generate. Resep Rawat Inap mengambil obat dari `Rencana Terapi` Pengantar Opname baru pada route `pengantar-opname-new` dan tidak memasukkan `Rencana Tindakan`; jenis rekam medis lama tetap dipakai sebagai fallback. Setiap alur e-Resep mewajibkan dokter memilih depo pengambilan; extension menyelaraskan pilihan itu ke dropdown `Ruangan` eRM sebelum Generate dan memeriksanya kembali sebelum item dimasukkan.
